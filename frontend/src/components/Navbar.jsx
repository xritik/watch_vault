    ...
    <>
      <nav id="mainNav">
        ...
        <div className="nav-right">
          <button className="ttoggle" onClick={toggleTheme} aria-label="Toggle theme">
            <span>{theme === 'dark' ? '🌙' : '☀️'}</span>
            <span className="toggle-label-text">{theme === 'dark' ? 'Dark' : 'Light'}</span>
      ...
      <div className={`mobile-drawer ${drawerOpen ? 'open' : ''}`}>
        ...
        <button className="drawer-theme-btn" onClick={toggleTheme}>
          ...
          <div className="tt-track" style={{ marginLeft: 'auto' }}>
            <div className="tt-thumb" id="drawerThumb"></div>
          </div>
        </button>
      </div>
      <div
